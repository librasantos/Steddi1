...
```
This runs hourly. The endpoint itself filters to only send to users whose local
timezone hits their chosen reminder hour in that run.

**Note:** Vercel cron is on the Pro plan ($20/mo). Hobby tier does NOT run crons.
If you're on Hobby, you can either:
- Upgrade to Pro
- OR use a free external cron service (e.g. cron-job.org) that POSTs to
  `https://your-app.vercel.app/api/reminder-cron` hourly with
  `Authorization: Bearer <your CRON_SECRET>` header
